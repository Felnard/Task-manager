...
//  default
$query = 'SELECT * FROM task WHERE userId = '. $userId. ' && trash = 1 order by dateTask';
$header = 'Trash';


if(isset($_POST['restore'])){
  $trashUpdate = "UPDATE task SET trash = 0 WHERE userId = $userId && id = $id";
  ...
  $query = 'SELECT * FROM task WHERE userId = '. $userId. ' && trash = 1 ORDER BY timeto, dateTask';
  $header = 'Trash';
  ...
  <body>
    ...
    <form method="POST"  action='home.php'>
      ...
      <h4 class='text-center text-primary'>Classification</h4>
        ...
        <button type="submit" name='<?php echo $classifications['classification']?>' class=" bg-transparent border-0">
         <span name='?php echo $classifications['classification']?>' >
           ?php 
    ...
    <div class="holder d-flex justify-content-center flex-wrap text-body mt-5"> 
        ...
        <section class="task-holder">
          <div class="info">
            <div>
             ...
             <input type="hidden" value="<?php echo $task['trash'] ?>" name='trash'>
                  
            <button class="star text-white bg-danger border border-white p-1 rounded-pill" type="submit" name="delete">Delete</button>
            <button class="star text-white bg-success border border-white p-1 rounded-pill" type="submit" name="restore">Restore</button>
             </form>
      ...
      <?php endforeach;?>  
             ...
             <!-- add task -->
            <?php
              echo addTasks($tasks);mysqli_close($conn);
             ?>
